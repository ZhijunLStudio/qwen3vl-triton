...
class VLMModel:
    ...
    def _optimize_llm_mlp(self):
        """
        Contribution 1: 物理层 SwiGLU 算子融合
        消除 Qwen3VLTextMLP 中同源输入的双重线性投影开销，减少访存 (Memory-Bound)
        """
        import torch
        import torch.nn.functional as F
        import types

        print("[VLMModel] Applying Physical Weight Fusion to LLM MLP...")

        # 我们自定义的极速 forward 函数
        def fused_mlp_forward(self_mlp, x):
            # 1. 只需要做一次矩阵乘法！(相当于原来的 gate 和 up 一起算了)
            # F.linear 直接调用底层 cuBLAS，极其高效
            fused_out = F.linear(x, self_mlp.fused_gate_up_weight)
            
            # 2. 在 GPU 的超高速缓存 (SRAM) 内部直接切片，不走慢速显存！
            gate, up = fused_out.chunk(2, dim=-1)
            
            # 3. 激活并相乘，最后过 down_proj
        ...
        if hasattr(self._model.model, 'language_model'):
            for layer in self._model.model.language_model.layers:
                ...
                # 提取出原始的权重数据 (这里假设没有使用 bitsandbytes 量化，纯 FP16/BF16)
                # 维度从 [6144, 2048] + [6144, 2048] -> 变成 [12288, 2048]
                weight_gate = mlp.gate_proj.weight.data
                weight_up = mlp.up_proj.weight.data
                fused_weight = torch.cat([weight_gate, weight_up], dim=0)
                
                # 将拼接后的权重注册为一个新的 Parameter
                mlp.register_parameter(
                    'fused_gate_up_weight', 
                    torch.nn.Parameter(fused_weight, requires_grad=False)
                )
            ...
            self._optimizations_applied.append('llm_mlp_fusion')
        else:
            print("[Warning] Could not find language_model to patch MLP.")
            
            
    def _optimize_kv_cache(self):
        """
        Contribution 2: 静态 KV Cache 预分配
        彻底消除动态生成过程中的 aten::copy_ 和显存碎片
        """
        print("[VLMModel] Enabling Static KV Cache Pre-allocation...")
        
        # 强制模型使用静态缓存机制
        # 这会在生成第一步时，直接 torch.empty 申请最大长度的显存
        # 后续生成全都使用指针切片赋值，不再有 torch.cat！
        self._model.generation_config.cache_implementation = "static"
        
        # 显式设定 pad_token_id 避免生成报错
        if getattr(self._model.config, 'pad_token_id', None) is None:
            self._model.config.pad_token_id = self._model.config.eos_token_id
            self._model.generation_config.pad_token_id = self._model.config.eos_token_id
